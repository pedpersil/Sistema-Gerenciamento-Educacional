...
if (!$disciplina->getDisciplineById($id)) {
...
if ($disciplina->deleteDiscipline($id)) {
    ...
    echo "<script>
            Swal.fire({
                  ...
                  if (result.isConfirmed) {
                      window.location.href = '" . BASE_URL . "admin/disciplinas.php';// Redireciona para a página de listagem
                  }
            });
          </script>";
} else {
    ...
    echo "<script>
            Swal.fire({
                title: 'Erro!',
                text: 'Não foi possível excluir a disciplina. Tente novamente.',
                icon: 'error',
                confirmButtonText: 'OK'
            }).then((result) => {
                if (result.isConfirmed) { 
