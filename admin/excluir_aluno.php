...
if ($aluno->deleteStudent($id)) {
    ...
    echo "<script>
            Swal.fire({
                  ...
                  if (result.isConfirmed) {
                      window.location.href = '" . BASE_URL . "admin/alunos.php';// Redireciona para a página de listagem
                  }
            });
          </script>";
} else {
    ...
    echo "<script>
            Swal.fire({
                title: 'Erro!',
                text: 'Não foi possível excluir o professoraluno. Tente novamente.',
                icon: 'error',
                confirmButtonText: 'OK'
            }).then((result) => {
                if (result.isConfirmed) { 
