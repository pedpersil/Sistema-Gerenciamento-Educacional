...
if (!$professor->getTeacherById($id)) {
  // Se o professor não existe, redireciona para index.php
  header('Location: ' . BASE_URL . 'admin/index.php');
  exit();
}
?>
...
if ($professor->deleteTeacher($id)) {
    ...
    echo "<script>
            Swal.fire({
                  ...
                  if (result.isConfirmed) {
                      window.location.href = '" . BASE_URL . "admin/professores.php';// Redireciona para a página de listagem
                  }
            });
          </script>";
} else {
    ...
    echo "<script>
            Swal.fire({
                title: 'Erro!',
                text: 'Não foi possível excluir o professor. Tente novamente.',
                icon: 'error',
                confirmButtonText: 'OK'
            }).then((result) => {
                if (result.isConfirmed) { 
